export default function (): CodeNodeReturn {
  ...
  const items = results.map((page: IDataObject) => {
    ...
    const titleEntry = Object.entries(props).find(([_k, v]) => v.type === 'title');
    const titleArr = titleEntry ? (titleEntry[1].title as IDataObject[]) : [];
    const taskName = titleArr.length > 0 ? titleArr.map((t: IDataObject) => t.plain_text).join('') : '(無題)';
    ...
    const templateText = richText.map((t: IDataObject) => t.plain_text).join('');
